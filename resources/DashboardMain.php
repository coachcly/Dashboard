...
global $SYSTEM_color, $SYSTEM_msg, $CHKSYSDATE_color, $KEYLCKPOS_color, $CUMPTFPKG_color, $ASPUSED_color, $DSKSTSERR_color, $WRKPRBERR_color, $QSYSMSGERR_color, $BATSTS_color, $BATSTSDAYS_color, $ERRLOGERR_color;
global $SAVSYS_days, $CHKSYSDATE_days, $QDATETIME_days, $QDATETIME_hours;
global $QDATETIME_color, $MXMON_color, $MXDGSTS_color, $MXDGERR_color, $MXAUDERR_color, $MXNFYERR_color, $OBJDELAY_color, $MXMSGW_color, $THRESHOLD_color, $SRCASPUSED_color, $TGTASPUSED_color;
global $sort, $orderby;
global $x, $tooltip; 
...
{ 
	...
	while ($row = db2_fetch_assoc($stmt))
	{
		...
		if ($username == 'DEMOUSER') { 
			$SYSTEM = 'Demo - '.trim($row['CHKSYSKEY']);
		} else {
		...
		// ERRLOGERR Errors
		if ($ERRLOGERR == '*YES') $ERRLOGERR_color = "#FF8080"; else $ERRLOGERR_color = null;
		...
		if ($SAVSYS_days >= 180) {
			...
			$SYSTEM_msg = "title=\"** Warning ** <br><br> Last SAVSYS: <strong>$SAVSYS <br>    $SAVSYS_days</strong> Days Old \"";
			if ($SAVSYS_days >= 365) {
				$SYSTEM_color = "#FF8080"; 
				//$SYSTEM_msg = "     ** Error ** \n\n Last SAVSYS: $SAVSYS \n    $SAVSYS_days Days Old ";
				$SYSTEM_msg = "title=\"** Error ** <br><br> Last SAVSYS: <strong>$SAVSYS <br> $SAVSYS_days</strong> Days Old \"";
			}
			if ($SAVSYS == '//0') {
				$SYSTEM_color = "#FF8080"; 
				//$SYSTEM_msg = "     ** Error ** \n\n Last SAVSYS: $SAVSYS \n    $SAVSYS_days Days Old ";
				$SYSTEM_msg = "title=\"** Error ** <br><br> Last SAVSYS <strong>not found.</strong> \"";
			}
		} else { 
			$SYSTEM_color = null;
			$SYSTEM_msg = null;
		...
		if($view == 'ERROR' && ($SYSTEM_color.$SYSTEM_msg.$CHKSYSDATE_color.$KEYLCKPOS_color.$CUMPTFPKG_color.$ASPUSED_color.$DSKSTSERR_color.$WRKPRBERR_color.$QSYSMSGERR_color.$BATSTS_color.$BATSTSDAYS_color.$ERRLOGERR_color != '')) {
			wrtseg("ListDetails");
			$x++;
		}
...
{ 
	...
	FMIMIX.SRCASPUSED,  
	FMIMIX.TGTASPUSED  
	from DASHBOARD/FMIMIX join DASHBOARD/FCHKSYS 
	...
	while ($row = db2_fetch_assoc($stmt))
	{
		...
		// *** Look for errors and change backgrounds  ***
		// MIMIX Monitor status
		if ($MXMON != '*ACTIVE') $MXMON_color = "#FF8080"; else $MXMON_color = null;
		// MIMIX Datagroup Status
		if ($MXDGSTS != '*ACTIVE' && $MXDGSTS != '') $MXDGSTS_color = "#FF8080"; else $MXDGSTS_color = null;
		...
		// No longer used, changed to 24 hour per Scott
		...
		$QDATETIME_hours = hourDiff(substr($row['QDATETIME'],0,4).'-'.substr($row['QDATETIME'],4,2).'-'.substr($row['QDATETIME'],6,2)." ".substr($row['QDATETIME'],8,2).':'.substr($row['QDATETIME'],10,2).':'.substr($row['QDATETIME'],12,2));
		if ($QDATETIME_hours >= 24) $QDATETIME_color = "#FF8080"; else $QDATETIME_color = null;
		...
		if($view == 'MIMIXERROR' && ($MXMON_color.$MXDGSTS_color.$MXDGERR_color.$MXAUDERR_color.$MXNFYERR_color.$OBJDELAY_color.$MXMSGW_color.$THRESHOLD_color.$SRCASPUSED_color.$TGTASPUSED_color != '')) {
			wrtseg("ListMXDetails");
			$x++;
		}
	}
...
//===============================================================================
function hourDiff($hrsdiff)
{    
	$d1 = strtotime($hrsdiff);
	$d2 = time();
	return floor(($d2-$d1)/3600);
}
        ...
        <table id="listtable" class="mainlist">
          <tr>
            ...
            <th width="100" title="Click to Sort" onClick="form1.sort.value = &quot;DSKSTSERR&quot;;
              ...
              Errors</th>
            <th width="103" title="Click to Sort" onClick="form1.sort.value = &quot;WRKPRBERR&quot;;
              return Submit();" onMouseOver="mover(this);" onMouseOut="mout(this);">WRKPRB<br>
              Errors</th>
            <th width="112" title="Click to Sort" onClick="form1.sort.value = &quot;QSYSMSGERR&quot;;
              return Submit();" onMouseOver="mover(this);" onMouseOut="mout(this);">QSYSMSG<br>
              Critical<br>Errors</th>
            <th width="74" title="Click to Sort" onClick="form1.sort.value = &quot;BATSTS&quot;;
              return Submit();" onMouseOver="mover(this);" onMouseOut="mout(this);">Cache<br>
              Battery<br>Status</th>
            <th width="74" title="Click to Sort" onClick="form1.sort.value = &quot;ERRLOGERR&quot;;
              return Submit();" onMouseOver="mover(this);" onMouseOut="mout(this);">Error<br>
              Log<br>Status</th>
            <!--<th width="107" title="Click to Sort" onClick="form1.sort.value = &quot;BATSTSDAYS&quot;;
              return Submit();" onMouseOver="mover(this);" onMouseOut="mout(this);">Cache<br>
              Battery<br>Status<br>(Days)</th>-->
            <th width="135" title="Click to Sort" onClick="form1.sort.value = &quot;CHKSYSKEY&quot;;
...
<tr class="$pf_altrowclr" id="$x" onmouseover="TRmover(this);" onmouseout="TRmout(this);">
  ...
  <td nowrap="nowrap" bgcolor="$SYSTEM_color" class="text $tooltip" $SYSTEM_msg><div align="left" onmouseover="mover(this);" onmouseout="mout(this);" onClick="ajaxCall('$CHKSYSKEY', '$x');"><strong>$SYSTEM</strong></div></td>
  ...
  <td nowrap="nowrap" bgcolor="$ERRLOGERR_color" class="text"  ><div align="center">$ERRLOGERR</div></td>
  ...
  <td nowrap="nowrap"  class="text">
    <div align="center"><strong>$CHKSYSKEY</strong>&nbsp;&nbsp;
      <a href="#"><img src="images/bar_chart-icon.gif" title="System Performance &#10; $SYSTEM " height="9" onClick="window.open('DBCPUusage.php?CHKSYSKEY=$CHKSYSKEY','_blank','width=1024,height=768,status=no,scrollbars=yes,resizable=yes');"></a> 	
      &nbsp;
      <a href="#"><img src="images/reports_icon.gif" title="Available Reports &#10; $SYSTEM " height="9" onClick="loadwindow('FREPORT.php?CHKSYSKEY=$CHKSYSKEY',650,650)"></a>
    </div> 
  </td>
</tr>
      ...
      <div id="contents">
        ...
        <table id="listtable" class="mainlist">
          <tr>
            ...
            <th width="80" title="Click to Sort" onClick="form1.sort.value = &quot;MXDGSTS&quot;;
              return Submit();" onMouseOver="mover(this);" onMouseOut="mout(this);">MIMIX<br>Datagroup<br>Status</th>
...
<tr class="$pf_altrowclr" id="$x" onmouseover="TRmover(this);" onmouseout="TRmout(this);">
  ...
  <td nowrap="nowrap" class="text">$METHOD</td>
  <td nowrap="nowrap" bgcolor="$QDATETIME_color" class="text" title="Last data received $QDATETIME_hours hours ago."><div align="center">$QDATETIME</div></td>
  ...
  <td nowrap="nowrap" bgcolor="$MXDGSTS_color" class="text"><div align="center">$MXDGSTS</div></td>
 
  
  <td nowrap="nowrap" bgcolor="$MXDGERR_color" class="text" onClick="loadwindow('DSPMXDG.php?CHKSYSKEY=$CHKSYSKEY&amp;BG=
SEGDTA;
 echo urlencode($MXDGERR_color); 
		echo <<<SEGDTA
',950,500)" ><div align="center" onmouseover="mover(this);"  onmouseout="mout(this);">
  $MXDGERR</div></td>
  
  <td nowrap="nowrap" bgcolor="$MXAUDERR_color" class="text" onClick="loadwindow('DSPMXAUD.php?CHKSYSKEY=$CHKSYSKEY&amp;BG=
SEGDTA;
 echo urlencode($MXAUDERR_color); 
		echo <<<SEGDTA
',750,500)"><div align="center" onmouseover="mover(this);"  onmouseout="mout(this);">
  $MXAUDERR</div></td>
  
  <td nowrap="nowrap" bgcolor="$MXNFYERR_color" class="text" onClick="loadwindow('DSPMXNFY.php?CHKSYSKEY=$CHKSYSKEY&amp;BG=
SEGDTA;
 echo urlencode($MXNFYERR_color); 
		echo <<<SEGDTA
',800,400)"><div align="center" onmouseover="mover(this);"  onmouseout="mout(this);">
  $MXNFYERR</div></td>
...
global $CHKSYSKEY,$SYSTEM,$CHKSYSRLS,$METHOD,$CHKSYSRCDS,$QUSREXTPGM,$PRCDATE,$PRCTIME,$CHKSYSDATE,$CHKSYSTIME,$QDATETIME,$TIMEZONE,$QSRLNBR,$HDWTYPE,$LPARTYPE,$QMODEL,$QPRCFEAT,$QCONSOLE,$QCTLSBSD,$QSTRUPPGM,$CONSOLETYP,$KEYLCKPOS,$PRCGRP,$SYSNAME,$OSRLS,$CUMPTFPKG,$CUMPTFSTS,$PARTID,$JOB,$USER,$NBR,$TOTASP,$ASPUSED,$SAVPREMISE,$WATCHDOG,$RTVPFRSTS,$MONDSKSTS,$MONPRBSTS,$MONQSYSMSG,$MONBATSTS,$MONERRLOG,$RCLSTG,$RCLSTGDUR,$SAVSYS,$SAVSECDTA,$SAVCFG,$SAVNONSYS,$SAVALLUSR,$SAVIBM,$MINISAVSYS,$DSKSTSERR,$WRKPRBERR,$QSYSMSGERR,$BATSTS,$BATSTSDAYS,$ERRLOGERR,$MIMIXSBS,$MXLIB,$MXRLS,$MXROLE,$MXMON,$MXSTSDATE,$MXSTSTIME,$CHKSYS;
global $MXKEY,$SYSTEM,$LASTUPDBY,$METHOD,$MXSTSRCDS,$MXMONRCDS,$USREXTPGM,$MXSTSRLS,$MXMONRLS,$PRCDATE,$PRCTIME,$QDATETIME,$SYSNAME,$OSRLS,$QSRLNBR,$PARTID,$MXLIB,$MXRLS,$MXSRC,$MXTGT,$RDB,$MXDGSTS,$MXDGERR,$MXAUDERR,$MXNFYERR,$MXSTSDATE,$MXSTSTIME,$MXSTSJOB,$MXSTSUSER,$MXSTSNBR,$MIMIXSBS,$MXMON,$MXMONDATE,$MXMONTIME,$MXMONJOB,$MXMONUSER,$MXMONNBR,$OBJDELAY,$MXROLE,$MXMSGW,$THRESHOLD,$SRCASPUSED,$SRCSYSASP,$TGTASPUSED,$TGTSYSASP,$CHKMXSTS,$STRMXMON;
	global $pf_scriptname;
	$pf_scriptname = 'DashboardMain.php';
...
$pf_liblLibs[1] = 'DASHBOARD';
    ...
    // Last Generated CRC: 6CF360BA C04037D7 8F95C66C 80747B67
    // Path: C:\ESDI\WebSmart\temp\Dashboard_www_websmart_htdocs_dashboard_PHW Filels_DashboardMain.phw
}
?>