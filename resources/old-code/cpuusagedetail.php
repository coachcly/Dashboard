...
//	Program Name:		cpuusagedetail.php
...
global $SBSJOB, $member, $sysname, $datein, $timein, $dayofweek, $datetime, $date_back;

// Set maximum list size to 30 for this program
$ww_listsize = 30;
...
if (isset($_REQUEST['sysname']))
	$sysname = $_REQUEST['sysname']; else $sysname = "** Warning - Could not allocate system";
...
-->
  ...
  <body background="images/back_train.jpg">
  ...
  <tr>
    ...
    <td><strong><span class="style1">$sysname</span></strong></td>
  </tr>
  <tr>
...
<br />
<div id="contents">
      <button onClick="javascript:location.href='DBCPUusage.php?datein=$date_back';">Go Back</button>
      ...
      <table id="listtable" class="mainlist">
        <tr valign="middle">
          ...
          <th width="100" nowrap="nowrap"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=INTRECNBR&rnd=$rnd">Subsystem/Job</a></th>
          <th width="100" nowrap="nowrap"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBUSER&rnd=$rnd">User</a></th>
          <th width="60" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBNBR&rnd=$rnd">Number</a></div></th>
          <th width="50" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBTYPE&rnd=$rnd">Type</a></div></th>
          <th width="50" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBPOOL&rnd=$rnd">Pool</a></div></th>
          <th width="50" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBRUNPTY&rnd=$rnd">Priority</a></div></th>
          <th width="70" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBCPUTIME&rnd=$rnd">CPU Time<br>
          in Seconds</a></div></th>
          <th width="80" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBTEMPSTG&rnd=$rnd">Temporary<br>
          Storage</a></div></th>
          <th width="60" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBAUXIO&rnd=$rnd">Auxilory<br>
          I/O</a></div></th>
          <th width="60" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBTHREADS&rnd=$rnd">Threads</a></div></th>
          <th width="60" nowrap="nowrap"><div align="right"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBCPUPRC&rnd=$rnd">CPU %</a>&nbsp;&nbsp;</div></th>
          <th width="150" nowrap="nowrap"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBFNC&rnd=$rnd">Function</a></th>
          <th width="60" nowrap="nowrap"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=JOBSTATUS&rnd=$rnd">Status</a></th>
          <th width="100" nowrap="nowrap"><a href="$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&ordby=CURUSER&rnd=$rnd">Current<br>
          User</a></th>
        </tr>
...
<script type="text/javascript">
	...
	if ($ww_prevpage > 0) 
	{
		...
		jQuery(".prevlink").attr("href", "$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&page=$ww_prevpage&rnd=$rnd");
	}
	...
	if ($ww_count == $ww_listsize) 
	{
		...
		jQuery(".nextlink").attr("href", "$pf_scriptname?CHKSYSKEY=$member&sysname=$sysname&date=$datein&time=$timein&page=$ww_nextpage&rnd=$rnd");
	}	
</script>
...
global $SYSNAME,$OSREL,$SYSSTSDATE,$SYSSTSTIME,$PRCTYPE,$TOTPART,$CURAVLPRC,$CURINTCPW,$CURMEM,$CURPARTID,$CURPRC,$CURPRCCPTY,$NBRUSRACT,$NBRUSRDSC,$NBRUSRRQS,$NBRUSRGRP,$NBRBCHMSGW,$NBRBCHRUN,$NBRBCHHLD,$NBRBCHSCD,$NBRBCHJOBQ,$ENDJOBSPLF,$CPUUSED,$DBUSED,$JOBUSED,$TASKUSED,$ELAPSETIME,$JOBSINSYS,$PERMADDR,$TEMPADDR,$ASP1SIZE,$ASP1USED,$TOTAUXSTG,$CURUNPRSTG,$MAXUNPRSTG,$NBRACTPOOL,$QACTJOB,$QADLACTJ,$QADLTOTJ,$QBASACTLVL,$QBASPOOL,$QDAYOFWEEK,$QPFRADJ,$QMAXACTLVL,$QMCHPOOL,$QMODEL,$QPRCFEAT,$QSRLNBR,$QTOTJOB,$QTSEPOOL;
	...
	$pf_scriptname = 'cpuusagedetail.php';
...
$pf_liblLibs[1] = 'DASHBOARD';
    ...
    // Last Generated CRC: 2BD3C540 32B88AEC 886AC351 9DFCCE87
    // Path: E:\cpuusagedetail.phw
}
?>